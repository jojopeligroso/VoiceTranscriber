...
- **iOS model lock** — On iOS/iPadOS (all browsers are WebKit), only the ~40 MB Tiny English model (`whisper-tiny.en`) can load; Base/Small exceed WebKit's per-tab WASM memory ceiling. The app force-selects Tiny English on iOS and greys out every other option in Model settings with a plain-language note. This is also the fix for the out-of-memory "stuck loading" loop a returning user could hit with a larger model.
